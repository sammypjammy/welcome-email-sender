...
3. Under **Authentication**, add a **Single-page application** redirect URI for the deployed app. For local development, add `http://localhost:5173`.
...
The supplied `clientId` and `tenantId` are set in `src/outlookConfig.js`. Set `redirectUri` there only when it should differ from the app's web origin.
